    ...
    public partial class AutoBuyViewModel : ObservableObject
    {
        ...
        private async void RunStart(CancellationToken token,UserModel ds,bool isBroadcastEnabled)
        {
            try
            {
                ...
                while (!token.IsCancellationRequested && !_isExit)
                {
                    ...
                    // ==============================================
                    string currentPrice = ScreenOcrHelper.RecognizeNumberFromScreen(ds.PriceRegionLeftX, ds.PriceRegionLeftY, ds.PriceRegionRightX, ds.PriceRegionRightY);
                    AddLog($"{DateTime.Now:HH:mm:ss} OCR 识别价格：{currentPrice}");
                    if (int.TryParse(currentPrice, out int current))
                    {
                    ...
                    if (isBroadcastEnabled&&int.TryParse(currentPrice,out int scendcurrent))
                    {
                        try
                        {
        ...
        private async void RunStart31(CancellationToken token, UserModel ds, bool isBroadcastEnabled)
        {
            try
            {
                ...
                bool success = int.TryParse(ScreenOcrHelper.RecognizeNumberFromScreen(ds.RemainingHaverCoinLeftX, ds.RemainingHaverCoinLeftY, ds.RemainingHaverCoinRightX, ds.RemainingHaverCoinRightY), out int FirstMoney);
                if (success)
                {
                    AddLog($"初始金额：{FirstMoney}");
                }
                else { AddLog(ScreenOcrHelper.RecognizeNumberFromScreen(ds.RemainingHaverCoinLeftX, ds.RemainingHaverCoinLeftY, ds.RemainingHaverCoinRightX, ds.RemainingHaverCoinRightY)); }
                // 死循环，直到 R键 或 外部停止
                while (!token.IsCancellationRequested && !_isExit)
                {
                    // 【按 R 键立即退出循环】—— 核心判断
                    ...
                    MouseService.MoveAndClick(ds.AccountX, ds.AccountY, ds.ClickDelay);
                    //识别余额
                    success = int.TryParse(ScreenOcrHelper.RecognizeNumberFromScreen(ds.RemainingHaverCoinLeftX, ds.RemainingHaverCoinLeftY, ds.RemainingHaverCoinRightX, ds.RemainingHaverCoinRightY), out int CurrentMoney);
                    //计算价格
